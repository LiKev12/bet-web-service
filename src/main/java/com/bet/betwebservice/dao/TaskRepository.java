...
public interface TaskRepository extends JpaRepository<TaskEntity, UUID> {
    ...
    @Query("SELECT \n" +
            ...
            "(LOWER(task.name) LIKE CONCAT('%',:filterByName,'%'))")
    ...
    @Query("SELECT task\n" +
            "FROM TaskEntity task\n" +
            "WHERE (BIN_TO_UUID(task.idPod) = :idPod) AND task.isArchived=FALSE")
    List<TaskEntity> getTasksByIdPod(@Param("idPod") String idPod);

    @Query("SELECT task\n" +
            "FROM TaskEntity task\n" +
            "LEFT JOIN StampTaskStampHasTaskEntity stamp_task_stamp_has_task on stamp_task_stamp_has_task.idTask = task.id\n" +
            "WHERE BIN_TO_UUID(stamp_task_stamp_has_task.idStamp) = :idStamp AND task.isArchived=FALSE")
    List<TaskEntity> getTasksByIdStamp(@Param("idStamp") String idStamp);
    ...
    @Query("SELECT \n" +
            ...
            "LEFT JOIN TaskUserTaskStarEntity task_user_task_star ON task_user_task_star.idTask = task.id\n" +
            "LEFT JOIN TaskUserTaskPinEntity task_user_task_pin ON task_user_task_pin.idTask = task.id\n" +
            "WHERE (task.idPod IS NULL AND BIN_TO_UUID(task.idUserCreate) = :idUser) AND task.isArchived=FALSE\n" +
            "GROUP BY task.id")
    List<TaskIndividualPropertiesDTO> getPersonalTasks(@Param("idUser") String idUser);
}
